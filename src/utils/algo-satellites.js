...
export function predictNextFlyover(
  ...
  const observerGd = { longitude: toRadians(lon), latitude: toRadians(lat), height: alt };
  const stepSec = 30;
  ...
  if (riseTime && peakTime && setTime) return { start: riseTime, peak: peakTime, end: setTime, maxElevation };
  return null;
}
...
export function generateFlyoverHistory(line1, line2, lat, lon, alt = 0, days = 3) {
  const history = [];
  let currentDate = new Date();

  for (let i = 0; i < days; i++) {
    const flyover = predictNextFlyover(line1, line2, lat, lon, alt, currentDate, 1440);
    if (!flyover) break;
  ...
  if (history.length === 0) {
    history.push({ start: null, peak: null, end: null, maxElevation: 0, status: "never" });
  }
